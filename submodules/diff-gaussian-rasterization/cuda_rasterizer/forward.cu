...
renderCUDA(
	...
	float* __restrict__ contrib_w,   // (H*W) float32
	int*   __restrict__ contrib_id  // (H*W) int32
	)
{
	...
	// if (inside) {
	// 	contrib_w[pix_id]  = 0.f;   // weight 初值 0
	// 	contrib_id[pix_id] = -1;    // id   初值 -1
	// }
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// ----- record max omega (alpha * T) -----
			// float w_cur = alpha * T;
			// if (w_cur > contrib_w[pix_id]) {
			// 	contrib_w[pix_id]  = w_cur;
			// 	contrib_id[pix_id] = collected_id[j];
			// }

			// ----- record max omega (alpha * T) -----
            // if (inside) // Ensure thread is processing a valid pixel
            // {
            //     atomicAdd(&contrib_w[collected_id[j]], T); // Add T_k to sum for this scene Gaussian
            //     atomicAdd(&contrib_id[collected_id[j]], 1); // Increment contribution count for this scene Gaussian
            // }

			if (inside) // Ensure thread is processing a valid pixel
            {
                // Atomically update contrib_w[scene_id] if current T is greater.
                atomicMax(&contrib_w[collected_id[j]], T); 
            }
