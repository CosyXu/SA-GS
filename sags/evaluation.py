...
class DefaultEvaluationProtocol(EvaluationProtocol):
    ...
    def render(self, method: Method, dataset: Dataset, embedding_index: int) -> RenderOutput:
        return method.render(dataset["cameras"].item(), embedding_index=embedding_index)
    ...
    *,
    ...
    with tqdm(desc=description, total=len(dataset["image_paths"]), dynamic_ncols=True) as progress:
        for val in save_predictions(output,
                                    (
                                        evaluation_protocol.render(method, dataset_index_select(dataset, [i]), embedding_index=embedding_index[Path(dataset["image_paths"][i]).name] if embedding_index is not None else None) 
...
def save_predictions(output: str, predictions: Iterable[RenderOutput], dataset: Dataset, *, nb_info=None) -> Iterable[RenderOutput]:
    ...
    def _predict_all(open_fn) -> Iterable[RenderOutput]:
        for i, (pred, (w, h)) in enumerate(zip(predictions, assert_not_none(dataset["cameras"].image_sizes))):
            ...
            pred_image = image_to_srgb(pred["color"], np.uint8, color_space=color_space, allow_alpha=allow_transparency, background_color=background_color)
            pred_raw_image = image_to_srgb(pred["raw_color"], np.uint8, color_space=color_space, allow_alpha=allow_transparency, background_color=background_color)
            ...
            with open_fn(f"raw_color/{relative_name.with_suffix('.png')}") as f:
                save_image(f, pred_raw_image)
            ...
            if "depth" in pred:
                ...
                depth_rgb = visualize_depth(pred["depth"])
