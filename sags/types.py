...
SA_PREFIX = os.path.expanduser(os.environ.get("SA_GS_PREFIX", "~/.cache/nerfbaselines"))

ColorSpace = Literal["srgb", "linear"]
CameraModel = Literal["pinhole", "affine", "opencv", "opencv_fisheye", "full_opencv"]
...
class GenericCameras(Protocol[TTensor_co]):
    ...
    @property
    def dates(self) -> Optional[TTensor_co]:
        """Dates, [N]"""
        ...
...
class GenericCamerasImpl(Generic[TTensor_co]):
    ...
    dates: Optional[TTensor_co] = None
    ...
    def __getitem__(self, index):
        return type(self)(
            ...
            dates=self.dates[index] if self.dates is not None else None,
        )
    ...
    def __setitem__(self, index, value: Self) -> None:
        ...
        if self.dates is not None:
            self.dates[index] = cast(TTensor_co, value.dates)
    ...
    def cat(cls, values: Sequence[Self]) -> Self:
        ...
        return cls(
            ...
            metadata=metadata,
            dates=xnp.concatenate([v.dates for v in values]),
        )
    ...
    def apply(self, fn: Callable[[TTensor_co, str], TTensor]) -> 'GenericCamerasImpl[TTensor]':
        return GenericCamerasImpl[TTensor](
            ...
            dates=fn(cast(TTensor_co, self.dates), "metadata") if self.dates is not None else None,
        )
...
class UnloadedDataset(_IncompleteDataset):
    ...
    depths: NotRequired[Optional[Union[np.ndarray, List[np.ndarray]]]]  # [N][H, W]
...
class Method(Protocol):
    ...
    @abstractmethod
    def render(self, camera: Cameras, *, options: Optional[Dict] = None, embedding_index = None) -> RenderOutput:  # [h w c]
        """
        Render images.
...
class EvaluationProtocol(Protocol):
    ...
    def render(self, method: Method, dataset: Dataset, embedding_index: int) -> RenderOutput:
        ...
