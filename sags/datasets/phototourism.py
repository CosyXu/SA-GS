...
def load_dfc2019_dataset(path: Union[Path, str], split: str, use_defined_split=True, **kwargs):
    path = Path(path)
    if split:
    ...
    if use_defined_split:
        def read_split(filename):
            cam_infos = []
            with open(filename, 'r') as f:
                for line in f:
                    # 提取每行的第一个序号
                    first_number = int(line.split()[0]) - 1
                    cam_infos.append(first_number)
            return cam_infos
        
        if split == "train":
            cameras_train_file = os.path.join(path, "split", "train.txt")
            indices = read_split(cameras_train_file)
        else:
            cameras_test_file = os.path.join(path, "split", "test.txt")
            indices = read_split(cameras_test_file)
            
        assert len(indices) > 0, f"No images found in {split} list"
        logging.info(f"Using {len(indices)}/{dataset_len} images from the defined {split} list")
    else:
...
class NerfWEvaluationProtocol(EvaluationProtocol):
    def __init__(self):
        from sags.evaluation import compute_metrics
    ...
    def render(self, method: Method, dataset: Dataset, embedding_index: int) -> RenderOutput:
        ...
        return method.render(dataset["cameras"].item(), options={"embedding": embedding, "outputs": "depth"})
