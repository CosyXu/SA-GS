...
def dataset_load_features(
    dataset: UnloadedDataset, features=None, supported_camera_models=None, load_depth=False
) -> Dataset:
    if features is None:
    ...
    if load_depth:
        for p in tqdm(dataset["image_paths"], desc="loading depths", dynamic_ncols=True):
            depth = np.load(p.replace("images", "depths")[:-4]+".npy").astype(np.float32)
            depths.append(depth)
    ...
    if supported_camera_models is not None and supported_camera_models != set(("pinhole", "affine")):
        raise RuntimeError(
...
def new_dataset(*,
                ...
                depths: Optional[Union[np.ndarray, List[np.ndarray]]] = None,  # [N][H, W]
