...
from ._colmap_utils import read_points3D_ply
...
def load_colmap_dataset(path: Union[Path, str],
    ...
    if load_points:
        if not (colmap_path / "points3D.bin").exists() and not (colmap_path / "points3D.txt").exists() and not (colmap_path / "points3D.ply").exists():
        ...
        elif (colmap_path / "points3D.ply").exists():
            points3D = read_points3D_ply(colmap_path / "points3D.ply")
        else:
            raise DatasetNotFoundError("Missing 'sparse/0/points3D.{bin,txt}' file in COLMAP dataset")
    ...
    for image in images.values():
        ...
        rotation = image.qvec.reshape(3, 3).astype(np.float64)
        ...
        camera_poses.append(w2c)

        i += 1

    if load_dates:
        image_dates, image_metas = [], []
        infos_path = Path("infos")
        infos_path = path / infos_path  # 这里的path应该已经定义
        # 获取infos_path目录下的所有文件，并按文件名排序
        for sun_path in sorted(infos_path.iterdir()):
            if sun_path.is_file():  # 仅处理文件，排除子目录
                with open(sun_path, "r", encoding="utf-8") as txt_file:
                    lines = txt_file.readlines()
                    image_dates.append(lines[2].strip())  # 获取每个文件的第三行内容并去掉空格
                    image_metas.append([lines[3].strip(), lines[4].strip(), lines[5].strip(), lines[0].strip(), lines[1].strip()])
        # print(image_dates)
    ...
    far = np.stack([np.linalg.inv(x)[:3, -1] for x in camera_poses], 0)
