...
def eval_few_custom(method: SAGS, logger: Logger, dataset: Dataset, split: str, step: int, evaluation_protocol: EvaluationProtocol):
    disable_tqdm = False
...
@click.command("train")
@click.option("--data", type=str, default="dataset/dfc2019/JAX_068")
@click.option("--output", type=str, default="output", help="Output directory")
@click.option("--verbose", "-v", is_flag=True)
@click.option("--debug", is_flag=True)
@click.option("--dataset-type", type=click.Choice(["dfc2019"]), default="dfc2019")
...
def train_command(
    ...
    assert config_overrides["config"] == "dfc2019.yml"
    ...
    train_dataset = datasets.dataset_load_features(train_dataset, supported_camera_models=supported_camera_models, load_depth=True)
    train_dataset["images"] = [x[..., :3] for x in train_dataset["images"]]
    train_images = train_dataset.get("images")
    test_dataset = datasets.dataset_load_features(test_dataset, supported_camera_models=supported_camera_models, load_depth=True)
    ...
    test_dataset_eval_few = datasets.dataset_index_select(test_dataset, [0, 1])

    config_overrides['scene'] = data.split('/')[-1]

    method = SAGS(
        checkpoint=None,
        # checkpoint=Path("output_4images/JAX_068_season_0/checkpoint-10000"),
    ...
    for step in (pbar := tqdm(range(num_iterations), miniters=10, desc="training", disable=debug)):
        ...
        if step % 100 == 0:
            ...
            pbar.set_postfix({ "train/loss": f"{acc_metrics_values['loss']:.4f}", "psnr": f"{acc_metrics_values['psnr']:.4f}", "gaussians": f"{acc_metrics_values['num_gaussians']:.0f}" })
