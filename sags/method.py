...
from .utils import get_month_index, depth_warping
...
def camera_project(cameras: GenericCameras[Tensor], xyz: Tensor) -> Tensor:
    ...
    # World -> Camera
    origins = np.linalg.inv(cameras.poses)[..., :3, 3]
    rotation = np.linalg.inv(cameras.poses)[..., :3, :3]
...
class UncertaintyModel(nn.Module):
    ...
    def __init__(self, config: Config):
        ...
        if self.config.uncertainty_backbone == "dinov2_base_sat":
            import types
            from transformers import Dinov2Config, Dinov2Model

            self.backbone = Dinov2Model.from_pretrained("weights/dinov2_base_sat")

            self.backbone.patch_size = 14    
            self.backbone.embed_dim  = 768   # hidden_size=768
            self.backbone.num_heads  = 12    # num_attention_heads=12

            def get_intermediate_layers(model, x: torch.Tensor, n=[0], reshape=True):
                out = model(
                    x, 
                    output_hidden_states=True, 
                    return_dict=True
                )
                hidden_states = out.hidden_states  # tuple, 每层 (B, seq_len, dim)
                
                feats_list = []
                for idx in n:
                    if idx == model.num_heads - 1:
                        h_idx = -1
                    else:
                        h_idx = idx

                    feats = hidden_states[h_idx]  # shape (B, seq_len, dim)

                    # reshape => (B, dim, H', W')
                    if reshape:
                        patch_feats = feats[:, 1:, :]  
                        B, N, D = patch_feats.shape
                        H_ = x.shape[2] // model.patch_size
                        W_ = x.shape[3] // model.patch_size
                        patch_feats = patch_feats.reshape(B, H_, W_, D)
                        # (B, H_, W_, D) -> (B, D, H_, W_)
                        patch_feats = patch_feats.permute(0, 3, 1, 2).contiguous()
                        feats_list.append(patch_feats)
                    else:
                        feats_list.append(feats)

                return feats_list

            self.backbone.get_intermediate_layers = types.MethodType(get_intermediate_layers, self.backbone)
        else:
    ...
    def _compute_losses(self, gt, prediction, prefix='', _cache_entry=None, iter=iter):
        ...
        if self.config.uncertainty_mode == "l2reg":
        ...
        elif self.config.uncertainty_mode == "dino+mssim-v2":
            loss_mult = 1 / (2 * uncertainty.pow(2))
            gt_down = dino_downsample(gt, max_size=350)
            prediction_down = dino_downsample(prediction, max_size=350)
            dino_cosine = self._compute_cosine_similarity(
            ...
            dino_part = (1 - dino_cosine.sub(0.5).div(0.5)).clip(0.0, 1)
            msssim_part = 1 - msssim(gt_down, prediction_down, min_size=80).unsqueeze(1).detach()
            uncertainty_loss = dino_part + msssim_part * dino_downsample(loss_mult, max_size=350)
            if loss_mult.shape[2:] != gt.shape[2:]:
                loss_mult = F.interpolate(loss_mult, size=gt.shape[2:], mode='bilinear', align_corners=False)
            loss_mult = loss_mult.clamp_max(3)
    ...
    def dino_loss(self, gt, prediction, _cache_entry=None):
        gt = gt.unsqueeze(0)
        ...
        dino_cosine = self._compute_cosine_similarity(
            ...
            _x_cache=_cache_entry).detach()
        dino_part = (1 - dino_cosine.sub(0.5).div(0.5)).clip(0.0, 1)
        return dino_part.mean()

    def get_loss(self, gt_image, image, prefix='', _cache_entry=None, iter=-1):
        gt_torch = gt_image.unsqueeze(0)
        image = image.unsqueeze(0)
        loss, metrics, loss_mult = self._compute_losses(gt_torch, image, prefix, _cache_entry=_cache_entry, iter=iter)
    ...
    def get_uncertainty(self, gt_image, _cache_entry=None):
        uncertainty = self(self._scale_input(gt_image.unsqueeze(0), self.config.uncertainty_dino_max_size), _cache_entry=_cache_entry)
        return uncertainty
...
def getProjectionMatrixFromAffine(w, h, fx, fy):
    z_sign = 1.0
    P = torch.zeros((4, 4))
    P[0, 0] = 2.0 * fx / w
    P[1, 1] = 2.0 * fy / h
...
def get_cameras_extent(cameras: Cameras):
    c2w = np.linalg.inv(cameras.poses)
...
def get_cameras_extent_new(initial_xyz):
    min_coords = np.min(initial_xyz, 0)
    max_coords = np.max(initial_xyz, 0)
    scene_diagonal = np.linalg.norm(max_coords - min_coords)
    return scene_diagonal * 1.1
...
def setup_pose_sampling(train_cameras):
    directions = []
    rotations = []
    translations_2d = []
    
    for cam in train_cameras:
        pose = cam.poses  # shape (4,4) or (3,4) depending on your data
        R = pose[:3, :3]
        t = pose[:3, 3]  # [t1, t2, t3]

        direction = R[2, :].copy()
        norm_d = np.linalg.norm(direction) + 1e-8
        direction /= norm_d
        ...
        rotations.append(R)
        translations_2d.append([t1, t2])
    
    directions = np.array(directions)       # shape (N, 3)
    rotations = np.array(rotations)       # shape (N, 3, 3)
    translations_2d = np.array(translations_2d)  # shape (N, 2)

    dir_mean = directions.mean(axis=0)
    dir_mean /= (np.linalg.norm(dir_mean) + 1e-8)
    
    # bounding box
    t_min = translations_2d.min(axis=0)
    t_max = translations_2d.max(axis=0)
    
    ellipse_params = {
        "directions": directions,
        "rotations": rotations,
        "translations_2d": translations_2d,
        "dir_mean": dir_mean,
        "t_min": t_min,
        "t_max": t_max,
    }
...
class EmbeddingModel(nn.Module):
    def __init__(self, config: Config):
        ...
        if config.season_enabled:
            self.mlp = nn.Sequential(
                nn.Linear(config.appearance_embedding_dim + config.season_embedding_dim + feat_in + 6 * self.config.appearance_n_fourier_freqs, 128),
                nn.ReLU(),
                nn.Linear(128, 128),
                nn.ReLU(),
    ...
    def forward(self, gembedding, aembedding, sembedding, color, viewdir=None):
        del viewdir  # Viewdirs interface is kept to be compatible with prev. version

        input_color = color
        if self.config.appearance_model_sh:
            color = color[..., :((self.config.appearance_sh_degree + 1) ** 2) * 3]
        else:
            color = color[..., :3]

        if self.config.season_enabled:
            inp = torch.cat((color, gembedding, sembedding, aembedding), dim=-1)
        else:
        ...
        if mul.shape[-1] < input_color.shape[-1]:
            higher = mul[..., :3].repeat(1, (input_color.shape[-1] - mul.shape[-1]) // 3)
            mul = torch.cat((mul, higher), dim=-1)
        return input_color * mul + offset


class EmbeddingModelPlusModified(nn.Module):
    def __init__(self, config: Config): 
        super().__init__()
        self.config = config
        
        self.color_feat_dim = 3 # RGB
        if config.appearance_model_sh:
            self.color_feat_dim = ((config.appearance_sh_degree + 1) ** 2) * 3

        self.fourier_feat_dim = 6 * self.config.appearance_n_fourier_freqs # gembedding 
        self.appearance_emb_dim = config.appearance_embedding_dim          # aembedding 
        self.season_emb_dim = config.season_embedding_dim                # sembedding 

        self.dim_reinject = self.fourier_feat_dim + self.appearance_emb_dim

        enc_l1_in_dim = self.color_feat_dim + self.fourier_feat_dim + self.season_emb_dim + self.appearance_emb_dim
        self.enc_l1 = nn.Linear(enc_l1_in_dim, 128)
        self.enc_relu1 = nn.ReLU()
        
        self.enc_l2 = nn.Linear(128 + self.dim_reinject, 128)
        self.enc_relu2 = nn.ReLU()

        dec_l1_in_dim = 128 + self.dim_reinject
        self.dec_l1 = nn.Linear(dec_l1_in_dim, 128)
        self.dec_relu1 = nn.ReLU()

        self.dec_l2 = nn.Linear(128 + self.season_emb_dim + self.dim_reinject, self.color_feat_dim * 2)
    ...
    def forward(self, gembedding, aembedding, sembedding, color, viewdir=None):
        ...
        reinject_features = torch.cat((gembedding, aembedding), dim=-1)

        inp_enc = torch.cat((color, gembedding, sembedding, aembedding), dim=-1)

        x = self.enc_l1(inp_enc)
        x = self.enc_relu1(x)

        x_cat_enc = torch.cat((x, reinject_features), dim=-1)
        enc_out = self.enc_l2(x_cat_enc)
        enc_out = self.enc_relu2(enc_out) 

        x_cat_dec1 = torch.cat((enc_out, reinject_features), dim=-1)
        x = self.dec_l1(x_cat_dec1)
        x = self.dec_relu1(x)

        x_cat_dec2 = torch.cat((x, sembedding, reinject_features), dim=-1)
        output_params = self.dec_l2(x_cat_dec2) 

        split_dim = color.shape[-1]
        offset, mul = torch.split(output_params * 0.01, [split_dim, split_dim], dim=-1)
...
class GaussianModel(nn.Module):
    ...
    season_embeddings: Optional[nn.Parameter]
    ...
    def __init__(self, 
        ...
        if self.config.season_enabled:
            self.register_parameter("season_embeddings", cast(nn.Parameter, nn.Parameter(torch.empty(0, config.season_embedding_dim, dtype=torch.float32, requires_grad=True))))
        else:
            self.season_embeddings = None
        ...
        if self.config.appearance_enabled:
            ...
            if not self.config.season_enabled:
                ...
                self.appearance_mlp = EmbeddingModelPlusModified(config)
        else:
    ...
    def _setup_optimizers(self):
        ...
        if self.season_embeddings is not None:
            l.append({'params': [self.season_embeddings], 'lr': config.season_embedding_lr, "name": "season_embeddings", "weight_decay": config.season_embedding_regularization})
        if self.embeddings is not None:
    ...
    def set_num_training_images(self, num_images, num_seasons):
        if self.appearance_embeddings is not None:
        ...
        if self.season_embeddings is not None:
            self._resize_parameter("season_embeddings", (num_seasons, self.season_embeddings.shape[1]))
            self.season_embeddings.data.normal_(0, 0.01)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys, error_msgs):
        ...
        if self.season_embeddings is not None:
            self._resize_parameter("season_embeddings", state_dict["season_embeddings"].shape)
    ...
    @torch.no_grad()
    def compute_affine_3D_filter(self, cameras: Cameras):
        pass
    ...
    def opacity_decay(self, factor=0.99):
        opacities = self.opacity_activation(self.opacities) * factor
        self.opacities.data = self.inverse_opacity_activation(opacities)
    ...
    def simple_split(self, idx, N=2):
        gaussians = self.get_gaussians()
        device = gaussians["xyz"].device
        scales = self.scaling_activation(self.scales)

        n_init_points = self.xyz.shape[0]
        selected_pts_mask = torch.zeros((n_init_points), device=device, dtype=torch.bool)
        # selected_pts_mask[torch.tensor(idx, device=device)] = 1.0
        selected_pts_mask[idx] = 1
    ...
    def _render_internal(self, 
        ...
        world_view_transform = torch.tensor(getWorld2View(R, T)).transpose(0, 1).to(device=device)
        projection_matrix = (getProjectionMatrixFromAffine(width, height, fx, fy).transpose(0, 1).to(device=device))
        ...
        if only_depth:
            ...
            dist = means3D[:, 2].unsqueeze(-1).repeat(1,3)
            depth = rasterizer(
                means3D=means3D,
                means2D=means2D,
                colors_precomp=dist,
        ...
        if not self.config.appearance_enabled or (self.config.appearance_separate_tuned_color and return_raw):
            ...
            raw_rendered_image, radii, accumulation, contrib_id, contrib_w = rasterizer(
        ...
        if self.config.appearance_enabled:
            ...
            if self.config.season_enabled:
                assert viewpoint_camera.dates is not None, "Image dates are required for training"
                if season_index is None:
                    season_index = get_month_index(viewpoint_camera.dates, self.config.scene)
                season_embedding_expanded = self.season_embeddings[season_index].repeat(len(means2D), 1)
            else:
                season_embedding_expanded = None

            colors_toned = self.appearance_mlp(self.embeddings, embedding_expanded, season_embedding_expanded, features).clamp_max(1.0)
            ...
            # print("3:", colors_toned.shape)

            # rendered_image, _radii, _accumulation = rasterizer(
            rendered_image, _radii, _accumulation, _contrib_id, _contrib_w = rasterizer(
            # rendered_image, _radii = rasterizer(
                means3D=means3D,
                means2D=means2D,
                colors_precomp=colors_toned,
                opacities=opacity,
                # opacities=opacity_toned,
                scales=scales,
                rotations=rotations,
                shs=None,
                cov3D_precomp=None)
            ...
            contrib_id = _contrib_id if contrib_id is None else contrib_id
            contrib_w = _contrib_w if contrib_w is None else contrib_w
...
class SAGS(Method):
    ...
    def __init__(self, 
        ...
        self.model = GaussianModel(self.config, train_dataset, train_dataset is not None).to(device)
        ...
        if self.config.use_warping and train_dataset is not None:
            self._virtual_start_idx = 0
            self._virtual_end_idx = self.config.num_virtuals
            stage_size = self.config.iterations // self.config.virtual_stages
            self._stage_split_iters = [i * stage_size + 1 for i in range(self.config.virtual_stages)]
            self._num_per_group = self.config.num_virtuals // self.config.virtual_stages

            if self.ellipse_params is None:
                self.ellipse_params = setup_pose_sampling(self.train_cameras)

            if test_dataset is not None:
               self.test_cameras = test_dataset["cameras"]
               self.ellipse_params_test = setup_pose_sampling(self.test_cameras)

            self.generate_virtual_cams()


        if self.config.lpips_loss and train_dataset is not None:  #vgg alex
    ...
    def _setup_train(self, train_dataset: Dataset, load_state_dict):
        ...
        self.cameras_extent = get_cameras_extent_new(train_dataset["points3D_xyz"])
        ...
        if train_dataset["depths"] is not None:
            self.train_depths = [torch.from_numpy(convert_image_dtype(img, np.float32)[None]) for img in train_dataset["depths"]]
        ...
        if self.checkpoint is None:
            ...
            self.model.set_num_training_images(len(train_dataset["cameras"]), self.config.season_nums)
        else:
            self.model.load_state_dict(load_state_dict)

        self._viewpoint_stack = []
        self._virtual_viewpoint_stack = []

        if self.config.season_enabled:
            print(self.config.scene)
            self.season_indexes = torch.tensor([get_month_index(cam.dates, self.config.scene) for cam in self.train_cameras], dtype=torch.long, device="cuda")
            # print(self.season_indexes)

        # self.model.compute_3D_filter(cameras=self.train_cameras)
        self.model.compute_affine_3D_filter(cameras=self.train_cameras)

    def generate_virtual_cams(self):
        v_num = len(self.train_cameras) * 3 if self.config.num_virtuals < 1 else self.config.num_virtuals

        self.virtual_ids = np.random.choice(len(self.train_cameras), v_num, replace=True)

        self.virtual_images, self.virtual_depths, self.virtual_masks = [], [], []
        self.virtual_poses = []

        std_list = [0.1]

        stage_boundaries = [i * self._num_per_group for i in range(self.config.virtual_stages)] + [len(self.virtual_ids)]

        with torch.no_grad():
            for i in range(self.config.virtual_stages):
                id_slice = self.virtual_ids[stage_boundaries[i]:stage_boundaries[i+1]]
                std = std_list[i]

                for j, id in enumerate(tqdm(id_slice, desc=f"Stage {i+1}/{self.config.virtual_stages}, std={std}")):

                    input_img = self.train_images[id]
                    mvs_depth = self.train_depths[id].squeeze(0)

                    input_extr = self.train_cameras[id].poses

                    f_x, f_y, c_x, c_y = self.train_cameras[id].intrinsics
                    input_intr =  torch.tensor([[f_x, 0, c_x],
                                                [0, f_y, c_y],
                                                [0, 0, 1]]).to("cuda").to(torch.float32)

                    
                    flag = input_extr[0][0] < 0.1

                    target_intr = input_intr

                    target_extr = self.getRandPose(id, 0.1, flag, scene=self.config.scene[0:3])

                    torch.cuda.empty_cache()
                    # get priors for unseen views by forward warping
                    warped_image, warped_depth, valid_mask = depth_warping(input_img, mvs_depth, input_intr, target_intr,
                                                                        input_extr[:3,:3], input_extr[:3,3], target_extr[:3,:3], target_extr[:3,3], 
                                                                        reversed_bilinear=True, warping_depth_map=True)

                    self.virtual_images.append(torch.from_numpy(np.moveaxis(convert_image_dtype(warped_image, np.float32), -1, 0)))
                    self.virtual_depths.append(torch.from_numpy(convert_image_dtype(warped_depth, np.float32)[None]))
                    self.virtual_masks.append(torch.from_numpy(convert_image_dtype(valid_mask, np.float32)[None]))

                    self.virtual_poses.append(target_extr)

    @classmethod
    def get_method_info(cls) -> MethodInfo:
        return MethodInfo(
            method_id="sa-gs",  # Will be filled by the registry
            required_features=frozenset(("color", "points3D_xyz")),
            supported_camera_models=frozenset(("pinhole", "affine")),
        )
    ...
    def getRandPose(self, idx, std=0.02, flag=0, mode='train', scene='JAX'):

        if mode == 'train':
            params = self.ellipse_params
        else:
            params = self.ellipse_params_test

        base_dir = params["directions"][idx].copy()
        
        new_dir  = base_dir + np.random.normal(0, std, size=3)
        new_dir /= (np.linalg.norm(new_dir) + 1e-8)

        if flag:
            up_approx = np.array([1, 0, 0], dtype=float)
        else:
            up_approx = np.array([0, 1, 0], dtype=float)

        side = np.cross(new_dir, up_approx)
        if np.linalg.norm(side) < 1e-3:
            up_approx = np.array([0, 0, 1], dtype=float)
            side = np.cross(new_dir, up_approx)
        side /= (np.linalg.norm(side) + 1e-8)
        up_prime = np.cross(side, new_dir)
        ...
        R = np.stack([side, up_prime, new_dir], axis=0)
        if np.linalg.det(R) < 0:
            R[1] = -R[1]

        if scene == 'JAX':
            t1 = np.random.uniform(params["t_min"][0], params["t_max"][0])
            t2 = np.random.uniform(params["t_min"][1], params["t_max"][1])
            t3 = 1e6
        elif scene == 'OMA':
            t1 = params["translations_2d"][idx][0].copy()
            t2 = params["translations_2d"][idx][1].copy()
            t3 = 1e6

        render_pose = np.eye(4)
        render_pose[:3, :3] = R
        render_pose[:3, 3]  = [t1, t2, t3]
    ...
    def optimize_embedding(
        ...
        assert (camera.camera_models == camera_model_to_int("pinhole")) or (camera.camera_models == camera_model_to_int("affine")), "Only pinhole/affine cameras supported"
    ...
    def render(self, camera: Cameras, options=None, embedding_index=None, **kwargs) -> RenderOutput:
        del kwargs
        camera = camera.item()
        device = self.model.xyz.device
        assert (camera.camera_models == camera_model_to_int("pinhole")) or (camera.camera_models == camera_model_to_int("affine")), "Only pinhole/affine cameras supported"
        ...
        self.model.eval()
        with torch.no_grad():
            # print(self.model.appearance_embeddings)
            _np_embedding = (options or {}).get("embedding", None)
            embedding = (
                torch.from_numpy(_np_embedding)
                if _np_embedding is not None else self.model.get_embedding(embedding_index)
            )
            del _np_embedding
            ...
            print(f"rendering time: {elapsed_time_s:.4f} sec")

            image = out["render"]
            image = torch.clamp(image, 0.0, 1.0).nan_to_num_(0.0)
            color = image.detach().permute(1, 2, 0).cpu().numpy()

            raw_image = out["raw_render"]
            raw_image = torch.clamp(raw_image, 0.0, 1.0).nan_to_num_(0.0)
            raw_color = raw_image.detach().permute(1, 2, 0).cpu().numpy()
    ...
    def train_iteration(self, step):
        ...
        pick_virtual_cam = False

        if self.config.use_warping:
            if iteration in self._stage_split_iters:
            # if 0:
                print("VIRTUAL CAMS UPDATED:", iteration)
                stage_idx = self._stage_split_iters.index(iteration)
                self._virtual_start_idx = stage_idx * self._num_per_group
                if stage_idx == self.config.virtual_stages - 1:
                    self._virtual_end_idx = len(self.virtual_images)
                else:
                    self._virtual_end_idx = (stage_idx + 1) * self._num_per_group
                self._virtual_viewpoint_stack = [] 

            if not self._virtual_viewpoint_stack:
                self._virtual_viewpoint_stack = list(range(self._virtual_start_idx, self._virtual_end_idx))

            pick_virtual_cam = (iteration % self.config.pick_virtual_interval == 0)

            if pick_virtual_cam:
                camera_virtual_id = self._virtual_viewpoint_stack.pop(randint(0, len(self._virtual_viewpoint_stack) - 1))
                virtual_source_id = self.virtual_ids[camera_virtual_id]
                virtual_viewpoint_cam = self.train_cameras[virtual_source_id]
                virtual_source_depth = self.train_depths[virtual_source_id]

                virtual_pose = self.virtual_poses[camera_virtual_id]

                virtual_embedding = self.model.get_embedding(train_image_id=virtual_source_id)

                virtual_render_pkg = self.model._render_internal(virtual_viewpoint_cam, config=self.config, embedding=virtual_embedding, kernel_size=self.config.kernel_size, render_depth=True, current_iter=iteration, pose=virtual_pose)
                virtual_image_raw, virtual_image, viewspace_point_tensor, visibility_filter, radii = virtual_render_pkg["raw_render"], virtual_render_pkg["render"], virtual_render_pkg["viewspace_points"], virtual_render_pkg["visibility_filter"], virtual_render_pkg["radii"]
                
                virtual_gt_image = self.virtual_images[camera_virtual_id].to(device)
                virtual_gt_depth = self.virtual_depths[camera_virtual_id].to(device)
                virtual_valid_mask = self.virtual_masks[camera_virtual_id].to(device)

                loss = 0.0
                metrics = {}
                sampling_mask = None

        if not pick_virtual_cam:
        # Pick a random Camera
            ...
            viewpoint_cam = virtual_viewpoint_cam
        ...
        if not pick_virtual_cam:
            ...
            render_pkg = self.model._render_internal(viewpoint_cam, config=self.config, embedding=embedding, kernel_size=self.config.kernel_size, render_depth=False, current_iter=iteration)
            image_toned: Tensor
            ...
            if self.model.uncertainty_model is not None:
                del loss_mult
                uncertainty_loss, metrics, loss_mult = self.model.uncertainty_model.get_loss(gt_image, image_toned.detach(), _cache_entry=('train', camera_id), iter=iteration)
                loss_mult = (loss_mult > 1).to(dtype=loss_mult.dtype)
            ...
            if self.config.lpips_loss: 
                loss += self.config.lambda_lpips * self.lpips_criteria(image_toned, gt_image).mean()


        if self.config.use_warping:
            if pick_virtual_cam:

                Ll1 = torch.nn.functional.l1_loss(virtual_image, virtual_gt_image, reduction='none')
                ssim_value = ssim(virtual_image_raw, virtual_gt_image, size_average=False)
                virtual_loss = (
                    (1.0 - self.config.lambda_dssim) * (Ll1 * virtual_valid_mask).mean() + 
                    self.config.lambda_dssim * ((1.0 - ssim_value) * virtual_valid_mask).mean()
                )
                loss += self.config.lambda_virtual * virtual_loss
        ...
        with torch.no_grad():
            if not pick_virtual_cam:
                ...
                contrib_w = render_pkg["contrib_w"]
                contrib_idx = render_pkg["contrib_id"]
            else:
                mse = ((virtual_image - virtual_gt_image) * virtual_valid_mask).pow_(2)
                contrib_w = virtual_render_pkg["contrib_w"]
                contrib_idx = virtual_render_pkg["contrib_id"]

            # print(contrib_w[10000], contrib_idx[100000])
            ...
            if self.config.opacity_decay and iteration > self.config.densify_from_iter:
                # self.config.densify_until_iter = self.config.iterations
                self.model.opacity_decay(factor=self.config.opacity_decay_factor)
